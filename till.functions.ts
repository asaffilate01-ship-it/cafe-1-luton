...
function requireEnv(name: string): string {
  ...
  if (!v)
    throw new Error(`Missing env var ${name}. Add it in Cloud secrets to enable SumUp payments.`);
  return v;
}
...
export async function refundSumUpTransaction(
  ...
  const res = await fetch(`${BASE}/v0.1/me/refund/${transactionId}`, {
    ...
    body: JSON.stringify(amount_cents ? { amount: Number((amount_cents / 100).toFixed(2)) } : {}),
  });
