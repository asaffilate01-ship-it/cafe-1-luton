    ...
    <div className="min-h-screen bg-secondary/40">
                ...
                <tr key={a.id} className="hover:bg-secondary/30">
                  ...
                  <td className="p-3">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-base font-bold tracking-widest text-muted-foreground">
                        ••••••••
                      </span>
                      {isAdmin && (
                        <button
                          onClick={async () => {
                            ...
                            toast.success(
                              `New code ${res.code} copied. It will not be shown again.`,
                              { duration: 15000 },
                            );
                            void refresh();
                          }}
                  ...
                  <td className="p-3">
                    <button
                      ...
                      className={`rounded-full px-3 py-1 text-xs font-semibold disabled:cursor-not-allowed disabled:opacity-60 ${a.active ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}
                    >
      ...
      {showCreate && isAdmin && (
        <CreateAccountModal
          ...
          onCreated={(a) => {
            setShowCreate(false);
            void navigator.clipboard.writeText(a.access_code);
            toast.success(
              `Tab created — code ${a.access_code} copied. It will not be shown again.`,
              { duration: 15000 },
            );
            void refresh();
          }}
        ...
        <div className="flex items-start justify-between gap-4 print:block">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Access code stored securely · {new Date().toLocaleDateString()}
            </p>
          </div>
          ...
          {onTab.map((o) => (
            <div key={o.id} className="mt-3 rounded-xl border border-border p-4 text-sm">
              <div className="flex justify-between font-semibold">
                ...
                <span>{money(Math.max(0, o.total_cents - o.refunded_cents))}</span>
              </div>
