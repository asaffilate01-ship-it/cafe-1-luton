...
function Admin() {
  ...
  useEffect(() => {
    async function load() {
      const { data } = await supabase
        .from("orders")
        .select(
          "id, order_number, status, payment_status, type, total_cents, refunded_cents, customer_name, customer_phone, created_at, scheduled_for, schedule_mode, driver_id",
        )
  ...
  async function doRefund(o: OrderRow) {
    ...
    const reason = window.prompt(
      "Reason for this refund (required for the audit log):",
      "Customer request",
    );
    if (!reason?.trim() || reason.trim().length < 5)
      return toast.error("A refund reason is required");
    try {
  ...
  async function doMarkPaid(o: OrderRow) {
    const method = window.prompt("Payment method: type cash or card", "cash")?.trim().toLowerCase();
    if (method !== "cash" && method !== "card") return;
    const reference =
      method === "card"
        ? window.prompt("External card-terminal receipt reference:", "")?.trim()
        : undefined;
    if (method === "card" && (!reference || reference.length < 4)) {
      return toast.error("A card receipt reference is required");
    }
    const reason = window
      .prompt("Reason for manually settling this order:", "Payment verified by manager")
      ?.trim();
    if (!reason || reason.length < 5) return toast.error("A reason is required");
    try {
      await markPaid({ data: { order_id: o.id, method, reference, reason } });
  ...
  useAlertOnIncrease(
    incoming.length,
    "New order · Cafe1",
    `${incoming.length} order${incoming.length === 1 ? "" : "s"} awaiting acceptance`,
  );
  ...
  if (!rolesLoading && user && !has("admin") && !has("staff")) {
    return (
      <div className="p-10 text-center text-muted-foreground">
        Not authorised. Ask an admin to grant you a role.
      </div>
    );
  }
          ...
          <section className="mt-6 rounded-2xl border-2 border-primary bg-primary-soft/40 p-4">
            <div className="mb-3 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold text-primary">
                🔔 New orders awaiting acceptance ({incoming.length})
              </h2>
            </div>
              ...
              {incoming.map((o) => (
                ...
                >
                  ...
                  <div className="mt-3 flex gap-2">
                    <button
                      onClick={() => setStatus(o, "preparing", "Accepted · sent to kitchen")}
                      className="flex flex-1 items-center justify-center gap-1 rounded-full bg-primary py-2 text-sm font-semibold text-primary-foreground hover:bg-primary-hover"
                    >
                    ...
                    {has("admin") && (
                      <button
                        onClick={() => void doRefund(o)}
                        className="rounded-full border border-border px-3 text-xs font-semibold hover:border-primary hover:text-primary"
                      >
                        Refund / reject
                      </button>
                    )}
                ...
                <tr key={o.id}>
                  ...
                  <td className="p-3 capitalize">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {o.schedule_mode === "scheduled" && o.scheduled_for
                        ? new Date(o.scheduled_for).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })
                        : "ASAP"}
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      {has("admin") &&
                        ...
                        o.payment_status !== "on_account" && (
                          <button
                            onClick={() => void doMarkPaid(o)}
                            className="rounded-full bg-secondary px-3 py-1 text-xs font-semibold hover:bg-primary-soft"
                          >
                            Mark paid
                          </button>
                        )}
                      {o.status === "paid" && (
                        <button
                          onClick={() => setStatus(o, "preparing", "Accepted")}
                      ...
                      {has("admin") &&
                        ...
                        o.refunded_cents < o.total_cents && (
                          <button
                            onClick={() => doRefund(o)}
