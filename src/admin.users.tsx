      ...
      <div className="grid h-screen place-items-center bg-white px-8 text-center text-neutral-900">
        <div>
          ...
          <p className="mt-3 text-2xl text-neutral-500">
            Scan with your phone, enter your voucher code and opt in —{" "}
  ...
  return (
    <div className="grid h-screen grid-cols-1 overflow-hidden bg-neutral-950 text-white lg:grid-cols-[1fr_38%]">
      <section className="flex min-h-0 flex-col p-8">
        ...
        <div className="mt-6 flex items-end justify-between gap-6 border-t border-white/10 pt-6">
          <span className="text-xl font-bold uppercase tracking-widest text-white/50">
            {count} item{count === 1 ? "" : "s"}
          </span>
          <div className="min-w-72 space-y-1 text-right">
            {(voucherCents > 0 || discountCents > 0) && (
              <>
                <p className="flex justify-between gap-8 text-lg text-white/55">
                  <span>Subtotal</span>
                  <span>{money(subtotal)}</span>
                </p>
                {voucherCents > 0 && (
                  <p className="flex justify-between gap-8 text-lg text-indigo-300">
                    <span>Voucher</span>
                    <span>−{money(voucherCents)}</span>
                  </p>
                )}
                {discountCents > 0 && (
                  <p className="flex justify-between gap-8 text-lg text-indigo-300">
            ...
            <p className="text-xl font-bold uppercase tracking-wider text-white/55">Amount due</p>
