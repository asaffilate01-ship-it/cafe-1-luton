...
export const Route = createFileRoute("/admin/reports")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Daily and weekly sales, top items and order mix for Cafe1.",
      },
...
function Reports() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: o } = await supabase
        .from("orders")
        .select(
          "id, order_number, created_at, type, source, status, payment_status, payment_method, subtotal_cents, discount_cents, promo_discount_cents, voucher_cents, delivery_fee_cents, total_cents, refunded_cents, customer_name, driver_id",
        )
        .gte("created_at", from)
      ...
      if (!cancelled) {
        setDrivers(
          Object.fromEntries(
            (profs ?? []).map((p) => [p.id, p.full_name || p.email || p.id.slice(0, 6)]),
          ),
        );
      }
    })();
  ...
  const counted = useMemo(
    () => orders.filter((o) => o.status !== "cancelled" && o.total_cents > o.refunded_cents),
    [orders],
  );

  const totals = useMemo(() => {
    const gross = counted.reduce((s, o) => s + o.total_cents - o.refunded_cents, 0);
    return {
      gross,
      ...
      refunded: orders.reduce((s, o) => s + o.refunded_cents, 0),
    };
  }, [counted, orders]);
  ...
  const byDay = useMemo(() => {
    ...
    for (const o of counted) {
      ...
      const cur = map.get(k);
      if (cur) {
        cur.total += o.total_cents - o.refunded_cents;
        cur.count += 1;
      }
    }
    return [...map.entries()];
  ...
  function exportCsv() {
    ...
    const rows = orders.map((o) => [
      ...
      (o.refunded_cents / 100).toFixed(2),
      ((o.total_cents - o.refunded_cents) / 100).toFixed(2),
    ]);
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Cancelled and refunded orders are excluded from totals.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={exportCsv}
              className="inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1.5 text-xs font-semibold hover:border-primary"
            >
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            {
              label: "Deliveries",
              value: String(counted.filter((o) => o.type === "delivery").length),
            },
          ].map((c) => (
          ...
          <div className="mt-4 flex h-40 items-end gap-1.5">
            {byDay.map(([day, v]) => (
              <div
                ...
                title={`${day}: ${money(v.total)} · ${v.count} orders`}
              >
                <span className="text-[10px] font-semibold text-muted-foreground">
                  {v.total ? money(v.total) : ""}
                </span>
                <div
                  className="w-full rounded-t bg-primary"
                  style={{ height: `${Math.max(2, (v.total / maxDay) * 100)}%` }}
                />
              ...
              {topItems.map(([name, v]) => (
                <li key={name} className="flex items-center justify-between gap-3">
                  ...
                  <span className="shrink-0 text-muted-foreground">
                    ×{v.qty} ·{" "}
                    <span className="font-semibold text-foreground">{money(v.revenue)}</span>
                  </span>
                </li>
              ))}
