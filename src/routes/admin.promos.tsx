...
export const Route = createFileRoute("/admin/promos")({
  head: () => ({
    meta: [{ title: "Promo codes — Cafe1 Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <RequireRole roles={["admin"]} next="/admin/promos">
      <AdminPromos />
    </RequireRole>
  ),
});
...
function AdminPromos() {
  ...
  const [form, setForm] = useState({
    code: "",
    description: "",
    discount_type: "percent" as Row["discount_type"],
    discount_value: "10",
    min_subtotal: "",
    max_uses: "",
    applies_to: "all" as Row["applies_to"],
    starts_at: "",
    expires_at: "",
  ...
  async function create(e: React.FormEvent) {
    ...
    if (form.discount_type !== "free_delivery" && value <= 0)
      ...
      <div className="mx-auto max-w-3xl px-4 py-10">
        ...
        >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <input
              ...
              className="h-11 rounded-xl border border-border bg-background px-4 font-mono uppercase"
            />
            <input
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, discount_type: e.target.value as Row["discount_type"] })
              }
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, applies_to: e.target.value as Row["applies_to"] })
              }
              className="h-11 rounded-xl border border-border bg-background px-3"
            >
        ...
        <ul className="mt-3 space-y-2">
          {(rows ?? []).map((r) => (
            ...
            >
              <span className="rounded-lg border border-dashed border-primary bg-primary-soft px-3 py-1 font-mono text-sm font-bold text-primary">
                {r.code}
              </span>
              <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold">
                  {fmtDiscount(r)} · {r.applies_to.replace("_", " ")}
                </p>
