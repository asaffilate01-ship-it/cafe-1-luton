...
export const Route = createFileRoute("/admin/banners")({
  head: () => ({ meta: [{ title: "Promo banners — Cafe1 Admin" }, { name: "robots", content: "noindex" }] }),
...
function AdminBanners() {
  ...
  useEffect(() => { if (!loading && !user) navigate({ to: "/admin/login", search: { next: "/admin/banners" } }); }, [loading, user, navigate]);
  ...
  const [form, setForm] = useState({
    title: "", subtitle: "", badge: "", cta_label: "Order now", cta_url: "/menu",
    bg_color: "oklch(0.55 0.22 27)", image_url: "", sort_order: 0,
  });
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-3xl px-4 py-10">
        <div className="flex items-center gap-3">
          <span className="grid h-10 w-10 place-items-center rounded-xl bg-primary-soft text-primary"><ImageIcon className="h-5 w-5" /></span>
          <div>
            <h1 className="font-display text-3xl font-bold">Promo banners</h1>
            <p className="text-sm text-muted-foreground">Hero carousel shown on the home & menu pages.</p>
          </div>
        </div>
        ...
        <form onSubmit={create} className="mt-8 space-y-3 rounded-2xl border border-border bg-card p-5">
          ...
          <input required maxLength={80} placeholder="Title — e.g. 20% off breakfast" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="h-11 w-full rounded-xl border border-border bg-background px-4" />
          <input maxLength={140} placeholder="Subtitle (optional)" value={form.subtitle} onChange={(e) => setForm({ ...form, subtitle: e.target.value })} className="h-11 w-full rounded-xl border border-border bg-background px-4" />
          <div className="grid gap-3 sm:grid-cols-3">
            <input maxLength={20} placeholder="Badge (e.g. NEW)" value={form.badge} onChange={(e) => setForm({ ...form, badge: e.target.value })} className="h-11 rounded-xl border border-border bg-background px-4" />
            <input placeholder="CTA label" value={form.cta_label} onChange={(e) => setForm({ ...form, cta_label: e.target.value })} className="h-11 rounded-xl border border-border bg-background px-4" />
            <input placeholder="CTA link (/menu, /cart…)" value={form.cta_url} onChange={(e) => setForm({ ...form, cta_url: e.target.value })} className="h-11 rounded-xl border border-border bg-background px-4" />
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="flex items-center gap-2 rounded-xl border border-dashed border-border p-3 text-sm">
              ...
              <span className="truncate text-xs text-muted-foreground">{form.image_url ? "Image ready" : "or use background colour →"}</span>
            </label>
            <input placeholder="Background colour (oklch/hex)" value={form.bg_color} onChange={(e) => setForm({ ...form, bg_color: e.target.value })} className="h-11 rounded-xl border border-border bg-background px-4" />
          </div>
          {form.image_url && (
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
              <img src={form.image_url} alt="preview" className="h-14 w-24 rounded object-cover" />
              <button type="button" onClick={() => setForm({ ...form, image_url: "" })} className="underline">Remove</button>
            </div>
          )}
        ...
        <ul className="mt-3 space-y-3">
          {(rows ?? []).map((r) => (
            <li key={r.id} className="flex items-start gap-3 rounded-2xl border border-border bg-card p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {r.cta_url && <p className="text-xs text-primary">{r.cta_label || "CTA"} → {r.cta_url}</p>}
              </div>
          ...
          {rows && rows.length === 0 && <p className="text-sm text-muted-foreground">No banners yet.</p>}
        </ul>
      </div>
    </div>
  );
