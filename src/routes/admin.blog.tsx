      ...
      <div className="mx-auto max-w-4xl px-4 py-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-muted-foreground">
              Write stories, recipes and news for the public blog.
            </p>
          </div>
          {editingId && (
            <button
        ...
        >
          ...
          <textarea
            maxLength={280}
            rows={2}
            placeholder="Excerpt (shown on card & share previews)"
            value={form.excerpt}
            onChange={(e) => setForm({ ...form, excerpt: e.target.value })}
            className="w-full rounded-xl border border-border bg-background p-3"
          />
          <input
            placeholder="Tags, comma separated (e.g. recipes, breakfast)"
            value={form.tags}
          ...
          <label className="flex items-center gap-3 rounded-xl border border-dashed border-border p-3 text-sm">
            ...
            <span className="truncate text-xs text-muted-foreground">
              {form.cover_url ? "Cover set" : "PNG/JPG, landscape works best"}
            </span>
          </label>
          {form.cover_url && (
          ...
          <textarea
            rows={12}
            placeholder={
              "Post body (plain text or simple markdown: # heading, ## subheading, - list, > quote)"
            }
            value={form.body_md}
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {editingId && (
              <button
                type="button"
                onClick={startNew}
                className="h-11 rounded-full border border-border bg-background px-5 font-semibold"
              >
                Cancel
              </button>
            )}
        ...
        <ul className="mt-3 space-y-3">
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  /blog/{r.slug} · {r.published ? "Published" : "Draft"}
                </p>
                {r.tags?.length > 0 && (
                  <p className="mt-0.5 truncate text-xs text-primary">{r.tags.join(" · ")}</p>
                )}
              </div>
          ...
          {rows && rows.length === 0 && (
            <p className="text-sm text-muted-foreground">No posts yet — create your first above.</p>
          )}
        </ul>
      </div>
    </div>
