  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Your account — Cafe1" },
      {
        ...
        content: "See your recent Cafe1 orders and manage your account.",
      },
...
function Account() {
  ...
  const { data: profile } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("profiles")
        .select(
          "loyalty_points, lifetime_points, full_name, drink_stamps, free_drinks_available, free_drinks_redeemed",
        )
        .eq("id", user!.id)
        .maybeSingle();
      return data;
  ...
  async function reorder(orderId: string) {
    ...
    toast.success(
      skipped ? `Added ${added} item(s) — ${skipped} unavailable` : "Added to your basket",
    );
    navigate({ to: "/cart" });
  }
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2">
          <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Earn 1 point per £1 spent. Coming soon: redeem for free items.
            </p>
          </div>
          <div className="rounded-2xl border border-border bg-card p-5">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Personal discounts are available on request and applied automatically at checkout once
              approved.
            </p>
          </div>
          <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5 sm:col-span-2">
            ...
            <div className="mt-3 flex gap-1.5">
              {Array.from({ length: 10 }, (_, n) => (
                <span
                  key={n}
                  className={`h-3 flex-1 rounded-full ${n < (profile?.drink_stamps ?? 0) ? "bg-primary" : "bg-primary/20"}`}
                />
              ))}
            </div>
            <p className="mt-2 text-xs text-muted-foreground">
              Buy 10 coffees or teas and your 11th is free — applied automatically at checkout.
              {(profile?.free_drinks_available ?? 0) > 0 &&
                ` You have ${profile?.free_drinks_available} free drink${(profile?.free_drinks_available ?? 0) > 1 ? "s" : ""} waiting!`}
            </p>
          </div>
        </div>
