...
ALTER TABLE public.customer_discounts DROP CONSTRAINT IF EXISTS customer_discounts_percent_range;
ALTER TABLE public.customer_discounts DROP CONSTRAINT IF EXISTS customer_discounts_type_check;
ALTER TABLE public.customer_discounts
  ADD CONSTRAINT customer_discounts_type_check CHECK (
    discount_type IN ('percent', 'fixed_amount', 'free_delivery')
  );
ALTER TABLE public.customer_discounts DROP CONSTRAINT IF EXISTS customer_discounts_value_check;
ALTER TABLE public.customer_discounts
  ADD CONSTRAINT customer_discounts_value_check CHECK (
    CASE discount_type
      WHEN 'percent' THEN percent > 0 AND percent <= 100
      WHEN 'fixed_amount' THEN amount_cents > 0 AND amount_cents <= 100000
      ELSE true
    END
  );

DROP FUNCTION IF EXISTS public.get_customer_discount(text);
CREATE FUNCTION public.get_customer_discount(_email text)
RETURNS TABLE(percent integer, label text, discount_type text, amount_cents integer)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $fn$
  SELECT
    CASE WHEN d.discount_type = 'percent' THEN d.percent ELSE 0 END,
    d.label,
    d.discount_type,
    CASE WHEN d.discount_type = 'fixed_amount' THEN d.amount_cents ELSE 0 END
...
REVOKE ALL ON FUNCTION public.get_customer_discount(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_customer_discount(text) TO service_role;

DROP FUNCTION IF EXISTS public.prepare_counter_order(
  uuid, uuid, text, text, text, text, text, text, text, jsonb
);

CREATE FUNCTION public.prepare_counter_order(
...
REVOKE ALL ON FUNCTION public.prepare_counter_order(
  uuid, uuid, text, text, text, text, text, text, text, jsonb, text, integer, text
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.prepare_counter_order(
  uuid, uuid, text, text, text, text, text, text, text, jsonb, text, integer, text
) TO service_role;

DROP FUNCTION IF EXISTS public.prepare_counter_order_secure(
  uuid, uuid, text, text, text, text, text, text, text, text, jsonb
);

CREATE FUNCTION public.prepare_counter_order_secure(
  _idempotency_key uuid,
  _shift_id uuid,
  _customer_name text,
  _order_type text,
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.prepare_counter_order(
    ...
    _manual_discount_type, _manual_discount_value, _manual_discount_reason
  );
END $fn$;

REVOKE ALL ON FUNCTION public.prepare_counter_order_secure(
  uuid, uuid, text, text, text, text, text, text, text, text, jsonb, text, integer, text
) FROM PUBLIC, anon;
