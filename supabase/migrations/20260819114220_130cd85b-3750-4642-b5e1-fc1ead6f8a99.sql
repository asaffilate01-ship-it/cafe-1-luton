...
begin
  ...
  if ord.status = 'cancelled' then return ord; end if;

  if not (
    (ord.payment_status = 'on_account' and ord.account_id is not null)
    or ord.payment_status <> 'paid'
    or ord.source in ('counter', 'sumup_pos', 'deliveroo', 'just_eat', 'uber_eats', 'tgtg')
  ) then
    raise exception 'That order was paid online — refund it instead of cancelling';
  end if;
  ...
  update public.orders set
    status = 'cancelled',
    payment_status = case when ord.payment_status = 'paid' then ord.payment_status else 'pending' end,
    account_id = null,
    cancelled_at = now(),
  ...
  insert into public.audit_events (
    ...
    jsonb_build_object('order_number', ord.order_number, 'reason', reason, 'total_cents', ord.total_cents)
  );
