<#
  Cafe 1 one-click Uber Eats watcher installer.
  ...
  Uber Eats credentials are entered only into Uber Eats Manager's own page.
#>

$ErrorActionPreference = "Stop"
$ProgressPreference = "SilentlyContinue"

$taskName = "Cafe1 Uber Eats Watcher"
$sourceDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$installDir = Join-Path $env:LOCALAPPDATA "Cafe1\UberEatsWatcher"
$sourceFiles = @(
  "ubereats-hub-watcher.mjs",
  "ubereats-hub-watcher.cmd",
  "watcher-runtime.cmd",
  "watcher-launcher.vbs",
  "install-ubereats-watcher.ps1",
  "START-CAFE1-UBEREATS.cmd",
  "REPAIR-UBEREATS-LOGIN.cmd",
  "CHECK-UBEREATS-STATUS.cmd",
  "package.json",
  "package-lock.json",
  "README-FIRST.txt"
)
...
function Stop-Watcher {
  ...
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      ($_.CommandLine -match "ubereats-hub-watcher" -or $_.CommandLine -match "watcher-launcher")
    } |
...
Write-Host "     CAFE 1 - UBER EATS TO KDS ONE-CLICK SETUP" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor DarkCyan
Write-Host "This keeps the Uber Eats tablet unchanged and mirrors accepted orders to KDS."
Write-Host "No Uber Eats password is stored by Cafe 1."
...
if ([System.IO.Path]::GetFullPath($sourceDir).TrimEnd('\') -ne [System.IO.Path]::GetFullPath($installDir).TrimEnd('\')) {
  ...
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $installDir "install-ubereats-watcher.ps1")
  exit $LASTEXITCODE
}
...
@{
  ...
  hubUrl = "https://merchants.ubereats.com/manager/"
  refreshMs = 45000
...
function Resolve-NodeRuntime {
  ...
  $temporary = Join-Path $env:TEMP "cafe1-ubereats-node-$version"
...
$env:UBEREATS_BRIDGE_SECRET = $bridgeKey
Write-Step "Checking the protected Cafe 1 KDS bridge"
& $nodeExe (Join-Path $installDir "ubereats-hub-watcher.mjs") --check
$bridgeReady = $LASTEXITCODE -eq 0
if (-not $bridgeReady) {
  $settings = "UBEREATS_INGEST_MODE=hub_watcher`r`nUBEREATS_BRIDGE_SECRET=$bridgeKey"
  ...
  Write-Host "The Uber Eats key is protected on this PC and is not an Uber Eats password."
  Write-Host ""
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Run START-CAFE1-UBEREATS.cmd again after the settings are live."
    exit 3
  }
}

Write-Step "Connecting the always-on Uber Eats device account"
Write-Host "A Microsoft Edge window will open. Sign into Uber Eats Manager once using the"
Write-Host "same device account used at the cafe. Do not type the password into this installer."
& $nodeExe (Join-Path $installDir "ubereats-hub-watcher.mjs") --setup
if ($LASTEXITCODE -ne 0) { throw "Uber Eats Manager sign-in did not complete." }
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger `
  ...
  -Description "Keeps accepted Uber Eats Manager orders flowing to Cafe 1 KDS." | Out-Null
Start-ScheduledTask -TaskName $taskName

$shell = New-Object -ComObject WScript.Shell
$desktop = [Environment]::GetFolderPath("Desktop")
foreach ($shortcut in @(
  @{ Name = "Cafe 1 Uber Eats Status.lnk"; Target = "CHECK-UBEREATS-STATUS.cmd" },
  @{ Name = "Repair Uber Eats Login.lnk"; Target = "REPAIR-UBEREATS-LOGIN.cmd" }
)) {
...
Write-Host "  CONNECTED - UBER EATS ORDERS WILL FLOW TO CAFE 1 KDS" -ForegroundColor Green
Write-Host "========================================================" -ForegroundColor DarkGreen
Write-Host "The watcher starts at Windows sign-in, stays hidden and restarts automatically."
Write-Host "Use the new Cafe 1 Uber Eats Status desktop shortcut at any time."
