...
set "LOGFILE=%~dp0logs\justeat-hub-watcher.log"

:loop
for %%A in ("%LOGFILE%") do if %%~zA GTR 5242880 (
  move /y "%LOGFILE%" "%LOGFILE%.previous" >nul 2>&1
)
echo [%date% %time%] starting Cafe 1 Just Eat watcher >> "%LOGFILE%"
call "%~dp0watcher-runtime.cmd" >> "%LOGFILE%" 2>&1
set "WATCHER_EXIT=%ERRORLEVEL%"
if "%WATCHER_EXIT%"=="20" (
  echo [%date% %time%] Just Eat login required; waiting five minutes >> "%LOGFILE%"
