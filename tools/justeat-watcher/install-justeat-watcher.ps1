<#
  Cafe 1 one-click Just Eat watcher installer.
  ...
  Just Eat credentials are entered only into Partner Centre's own page.
#>
...
function Stop-Watcher {
  ...
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object {
      ...
      ($_.CommandLine -match "justeat-hub-watcher" -or $_.CommandLine -match "watcher-launcher")
    } |
...
if ([System.IO.Path]::GetFullPath($sourceDir).TrimEnd('\') -ne [System.IO.Path]::GetFullPath($installDir).TrimEnd('\')) {
  ...
  & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $installDir "install-justeat-watcher.ps1")
  exit $LASTEXITCODE
}
...
@{
  ...
  hubUrl = "https://partner.just-eat.co.uk/orders"
  refreshMs = 45000
...
function Resolve-NodeRuntime {
  ...
  $temporary = Join-Path $env:TEMP "cafe1-justeat-node-$version"
...
& $nodeExe (Join-Path $installDir "justeat-hub-watcher.mjs") --check
$bridgeReady = $LASTEXITCODE -eq 0
if (-not $bridgeReady) {
  $settings = "JUSTEAT_INGEST_MODE=hub_watcher`r`nJUSTEAT_BRIDGE_SECRET=$bridgeKey"
  ...
  Write-Host "The Just Eat key is protected on this PC and is not a Just Eat password."
  Write-Host ""
  ...
  if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "Run START-CAFE1-JUSTEAT.cmd again after the settings are live."
    exit 3
  }
}

Write-Step "Connecting the always-on Just Eat device account"
Write-Host "A Microsoft Edge window will open. Sign into Partner Centre once using the"
Write-Host "same device account used at the cafe. Do not type the password into this installer."
& $nodeExe (Join-Path $installDir "justeat-hub-watcher.mjs") --setup
if ($LASTEXITCODE -ne 0) { throw "Just Eat Partner Centre sign-in did not complete." }
...
Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger `
  ...
  -Description "Keeps accepted Just Eat Partner Centre orders flowing to Cafe 1 KDS." | Out-Null
Start-ScheduledTask -TaskName $taskName

$shell = New-Object -ComObject WScript.Shell
$desktop = [Environment]::GetFolderPath("Desktop")
foreach ($shortcut in @(
  @{ Name = "Cafe 1 Just Eat Status.lnk"; Target = "CHECK-JUSTEAT-STATUS.cmd" },
  @{ Name = "Repair Just Eat Login.lnk"; Target = "REPAIR-JUSTEAT-LOGIN.cmd" }
)) {
