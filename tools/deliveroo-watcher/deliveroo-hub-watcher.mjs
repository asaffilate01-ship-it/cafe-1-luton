#!/usr/bin/env node
/**
 * Cafe 1 Deliveroo Hub watcher.
 *
 * Uses a dedicated persistent Edge profile. The operator signs into
 * Restaurant Hub once in a real browser window; the watcher never reads or
 * stores the Deliveroo username or password. Accepted Hub payloads are copied
...
const PROFILE_DIR = path.join(SCRIPT_DIR, ".deliveroo-edge-profile");
...
const SECRET = process.env.DELIVEROO_BRIDGE_SECRET?.trim();
...
const ENDPOINT = `${CONFIG.cafe1Url}/api/public/deliveroo/hub-ingest`;
...
function claimProcessLock() {
  try {
    ...
    try {
      ...
      console.error("Cafe 1 Deliveroo watcher is already running.");
      return false;
    } catch {
...
async function send(payloadText, source) {
  const response = await fetch(ENDPOINT, {
    ...
    headers: {
      ...
      "user-agent": "Cafe1-Deliveroo-Watcher/2",
    },
    body: payloadText,
...
if (SETUP) {
  console.log("");
  console.log("A Deliveroo Restaurant Hub window is open.");
  ...
  if (!connected) {
    console.error("Deliveroo sign-in was not completed within 15 minutes.");
    await context.close();
    releaseLock();
    process.exit(4);
  }
  ...
  console.log(
    bridgeReady
      ? "Deliveroo login saved and Cafe 1 KDS connected."
      : "Deliveroo login saved, but the website bridge key still needs configuring.",
  );
  await context.close();
  releaseLock();
...
if (await signedOut()) {
  ...
  fs.writeFileSync(
    LOGIN_REQUIRED_FILE,
    "Deliveroo Restaurant Hub needs signing in again. Double-click REPAIR-DELIVEROO-LOGIN.cmd.\r\n",
    { mode: 0o600 },
  );
  console.error("Deliveroo login is required. Run REPAIR-DELIVEROO-LOGIN.cmd.");
  await context.close();
  releaseLock();
  process.exit(20);
}
...
context.on("response", async (response) => {
  ...
  if (!isDeliverooUrl(url) || ignored.test(url)) return;
...
page.on("websocket", (socket) => {
  if (!isDeliverooUrl(socket.url())) return;
...
const refreshTimer = setInterval(async () => {
  ...
  try {
    ...
    if (await signedOut()) {
      ...
      console.error("Deliveroo session ended. Login repair is required.");
