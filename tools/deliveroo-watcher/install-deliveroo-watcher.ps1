...
function Protect-InstallAccess {
  # Use well-known SIDs rather than localized account names and grant access
  # before removing inheritance, so ACL hardening cannot lock out the user.
  ...
  $watcherPath = Join-Path $installDir "deliveroo-hub-watcher.mjs"
  try {
