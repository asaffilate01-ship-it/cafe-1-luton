...
const sourceRoot = resolve(root, "tools/ubereats-watcher");
const archivePath = resolve(root, "public/downloads/cafe1-ubereats-watcher-windows.zip");
const required = [
...
for (const name of Object.keys(archive)) {
  if (
    /password|\.env$|bridge-secret|watcher\.config|node_modules|\.runtime|\.ubereats-edge-profile/i.test(
      name,
    )
  ) {
...
const watcher = source.get("ubereats-hub-watcher.mjs")?.toString("utf8") ?? "";
for (const marker of [
  "launchPersistentContext",
  "isUberEatsUrl",
  "Cafe1-UberEats-Watcher/2",
  "LOGIN-REQUIRED.txt",
  "heartbeat: true",
]) {
...
for (const forbidden of ["DEVICE_PASSWORD", "HUB_PASSWORD", 'input[type="password"].fill']) {
  if (watcher.includes(forbidden))
    failures.push(`watcher must not store or enter credentials: ${forbidden}`);
}

const installer = source.get("install-ubereats-watcher.ps1")?.toString("utf8") ?? "";
for (const marker of [
...
for (const forbidden of ["Protect-InstallAccess", "/inheritance:r"]) {
  if (installer.includes(forbidden))
    failures.push(`installer must not remove inherited user access: ${forbidden}`);
}
...
if (failures.length) {
  console.error(
    `Uber Eats watcher package verification failed:\n${failures.map((item) => `- ${item}`).join("\n")}`,
  );
  process.exitCode = 1;
} else {
  console.log(`Uber Eats watcher package verification passed for ${required.length} files.`);
}
